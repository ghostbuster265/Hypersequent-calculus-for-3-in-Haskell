...
l3_mixingruleHypL xs ls rs (m1:m2:m3:[]) = [L3S (l3_insertcontextL ls (L3 ([],[],[],[])), l3_insertcontextR rs (L3 ([],[],[],[])))] ++ (delete m3 (delete m2 (delete m1 xs)))

l3_mixingruleHypR :: [L3Seq] -> [For] -> [For] -> [L3Seq] -> [L3Seq]
l3_mixingruleHypR xs ls rs (m1:m2:m3:[]) = [L3S (l3_insertcontextL ls (L3 ([],[],[],[])), l3_insertcontextR rs (L3 ([],[],[],[])))] ++ (delete m3 (delete m2 (delete m1 xs)))
...
l3_contextseq (L3S (L3 (_, y, z, i),L3 (_, y', z', _))) = if null (y ++ z ++ y' ++ z') && not (null i) then True else False
...
l3_mixcontexts [L3S (L3 (xs1,ys1,zs1,is1), L3 (xs1',ys1',zs1',is1')),L3S (L3 (xs2,ys2,zs2,is2), L3 (xs2',ys2',zs2',is2')),L3S (L3 (xs3,ys3,zs3,is3), L3 (xs3',ys3',zs3',is3'))] =
	rmdups [((ll1 ++ ll2 ++ ll3, lr1 ++ lr2 ++ lr3), (rl1 ++ rl2 ++ rl3, rr1 ++ rr2 ++ rr3)) |
			...
			seplegal ((ll1 ++ ll2 ++ ll3) ++ (lr1 ++ lr2 ++ lr3) ++ (rl1 ++ rl2 ++ rl3) ++ (rr1 ++ rr2 ++ rr3)) ((xs1 ++ ys1 ++ zs1 ++ is1) ++ (xs2 ++ ys2 ++ zs2 ++ is2)
			++ (xs3 ++ ys3 ++ zs3 ++ is3) ++ (xs1' ++ ys1' ++ zs1' ++ is1') ++ (xs2' ++ ys2' ++ zs2' ++ is2') ++ (xs3'++ ys3' ++ zs3' ++ is3'))]
...
l3_buildTrees (t:ts)
    ...
    | l3_mixTree t = case () of
        () | not (null (l3_derHyp (l3_mixedTrees t (l3_pick3 t)))) -> (l3_mixedTrees t (l3_pick3 t)) ++ ts
           | l3_contextTree t -> if null (l3_derHyp (l3_altTrees t)) then [l3_tripleTree t] ++ ts else (l3_altTrees t) ++ ts
...
l3_matchingformulas (x:xs) ys = x `elem` ys || l3_matchingformulas xs ys
